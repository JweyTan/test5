...
import CriticalIllnessPlusSummaryPane from './criticalIllnessPlus-summary-pane';
import Datepicker from 'vuejs-datepicker';
//import Swiper from 'swiper/js/swiper.js';

Vue.component('criticalillnessplus-summary-pane', CriticalIllnessPlusSummaryPane)
/**
...
const m3paform = new Vue({
    ...
    data: {
        ...
        prescreenYesNo:true,
        prescreenYesNo2:false,
        prescreenYesNo3:false,
        prescreenYesNo4:false,
        bmiHeight:'',
        bmiWeight:'',
        bmi:'',
    ...
    methods: {
        setPrescreenQuestion:function(name,value){
            this.prescreenYesNo = false;
            this.prescreenYesNo2 = false;
            this.prescreenYesNo3 = false;
            this.prescreenYesNo4 = false;
	        console.log("edit in git hahaha")
        ...
        calculatePremium: function () {
            var _this=this;
            console.log("this.formData['2'].formattedCoverageStartDate: "+this.formData['2'].formattedCoverageStartDate);
            console.log("this.formData[2].coverageStartDate: "+this.formData['2'].coverageStartDate);
            ...
            return $
                .ajax({
                    ...
                    data: {
                        ...
                        coveragePlan: "CTI OPT2 P30",
                        formula: 'critical-illness-plus',
                ...
                .promise().done(function(data){
                    _this.formData['2'].planSelected = true;
                    var dataParse = JSON.parse(data);
                    _this.formData['2'].basePremium = dataParse.grossPremium.toFixed(2);
                    _this.formData['2'].discountAmount = dataParse.discountAmount.toFixed(2);
                    _this.formData['2'].discountRate =dataParse.discountRate.toFixed(0);
        ...
        scrollTop() {
            return new Promise((resolve, reject) => {
                const offset = $("#criticalIllnessPlus-form").offset().top;
    ...
    computed: {
        bmiComputed:function(){
            var numheight =this.bmiHeight;
            var numweight =this.bmiWeight;
            return parseFloat(numheight) + parseFloat(numweight);
        },
    ...
    watch: {
        "formData.2.planName":function(e){
            console.log("chgange plan")
            this.calculatePremium();
        },
        "formData.2.coverageStartDate": function(e){
            console.log("chg start date");
            let startD = new Date(this.formData['2'].coverageStartDate);
            this.formData['2'].formattedCoverageStartDate = String(startD.getDate()).padStart(2,'0') + "/" +String(startD.getMonth()+1).padStart(2,'0') + "/"+ String(startD.getFullYear());
            this.formData['2'].dateSelected = true;
            this.calculatePremium();
            //.format('DD/MM/YYYY (ddd)')
            this.formData['2'].coverageEndDate= new Date(moment(this.formData[2].coverageStartDate).add(1, 'years'));
